...
import { Text } from "../../promo/Text"

export default function Model({ ...props }) {
  const group = useRef()
  const { nodes, materials, animations } = useGLTF('/anim_wave.glb')
  const { actions } = useAnimations(animations, group)

  useEffect(() => {actions['Armature.002|mixamo.com|Layer0'].play()}); 
